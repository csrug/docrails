...
Povšimněte si, že třída +Post+ dědí od třídy +ActiveRecord::Base+. Active Record poskytuje modelům v Rails spoustu funkcionality: základní operace se záznamy v databázi (CRUD, _Create, Read, Update, Destroy_), validační logiku, stejně jako podporu pro složité dotazy do databáze nebo propojení mezi jednotlivými modely.

Další důležitou částí je řádek +attr_accessible :content, :name, :title+. Ten definuje atributy, které mohou být hromadně upraveny (např. prostřednictvím _update_attributes_).
...
Blok předaný metodě +respond_to+ zpracovává HTML a JSON výstup akce +index+. Pokud načtete v prohlížeči stránku "http://localhost:3000/posts.json":http://localhost:3000/posts.json, uvidíte výpis příspěvků ve formátu JSON.
...
Všimněte si také, že nesmíme zapomenout přidat _:tags_attributes_ do seznamu povolených +attr_accessible+. Bez toho by při pokusu o úpravu štítků z modelu Post docházelo k výjimce MassAssignmentSecurity.
